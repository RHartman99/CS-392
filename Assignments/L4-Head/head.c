/*******************************************************************************
 * Name        : head.c
 * Author      : Ryan Hartman & Andrew Pantera
 * Date        : 2/14/20
 * Description : Practice with file system calls
...
int main(int argc, char *argv[])
{
    ...
    if (n_value)
    {
        if (!is_integer(n_value) || !get_integer(n_value, &line_count) || line_count < 0)
        {
            ...
            line_count = DEFAULT_LINE_COUNT; // Added to use default line count
        }
    }
    if (optind + 1 < argc)
    {
    ...
    char buf[1]; // 1 byte for read character and 1 for null terminator
    int lines_read = 0;
    while (read(src_fd, buf, 1) > 0 && lines_read < line_count)
    {
        if (buf[0] == '\n')
        {
            lines_read++;
    ...
    if (close(src_fd) < 0)
    {
        fprintf(stderr, "Error: Cannot close source file '%s': %s\n", src_file, strerror(errno));
        return EXIT_FAILURE;
    }
